...
<section class="projects-section">
    <div class="auto-container">
        <div class="">
            <div class="row">
                @foreach($licenses as $portfolio)
                <div class=" project-block col-lg-4 col-md-6 col-sm-12">
                    <div class="inner-box">
                        <div class="image-box">
                            <figure class="image"><a href="{{ route('license',$portfolio->slug) }}"><img src="{{ asset('uploads/'.$portfolio->photo) }}" alt="{{ $portfolio->name }}"></a></figure>
                        </div>
                        <div class="content-box">
                            <a href="{{ route('license',$portfolio->slug) }}" class="icon"><i class="fa @if(session('sess_lang_direction') == 'Right to Left (RTL)') fa-long-arrow-alt-left @else fa-long-arrow-alt-right @endif"></i></a>
                            <h4 class="title"><a href="{{ route('license',$portfolio->slug) }}" title="{{ $portfolio->name }}">{{ $portfolio->name }}</a></h4>
                        </div>
                    </div>
                </div>
                @endforeach
